...
describe("TxError", () => {
  ...
  it("translates insufficient balance error", () => {
    render(<TxError txResult={{ status: "failed", error: "Insufficient balance for transfer" }} />);
    expect(screen.getByText(/Insufficient balance to complete/)).toBeInTheDocument();
  });

  it("translates nonce error", () => {
    render(<TxError txResult={{ status: "failed", error: "Invalid nonce" }} />);
    expect(screen.getByText(/nonce mismatch/)).toBeInTheDocument();
  });

  it("translates signature error", () => {
    render(<TxError txResult={{ status: "failed", error: "Signature verification failed" }} />);
    expect(screen.getByText(/signature was rejected/)).toBeInTheDocument();
  });

  it("translates cancelled error", () => {
    render(<TxError txResult={{ status: "failed", error: "User cancelled the request" }} />);
    expect(screen.getByText(/cancelled the transaction/)).toBeInTheDocument();
  });

  it("shows raw error for unknown errors", () => {
    render(<TxError txResult={{ status: "failed", error: "Custom error code 42" }} />);
  ...
  it("renders Try Again button when onRetry is provided", () => {
    ...
    render(
      <TxError txResult={{ status: "failed", error: "something broke" }} onRetry={mockRetry} />
    );
