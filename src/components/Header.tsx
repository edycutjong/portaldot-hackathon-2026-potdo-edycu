      ...
      <div className="flex items-center gap-3">
        {connected && address ? (
          <div className="flex items-center gap-3">
            ...
            {accounts.length > 1 ? (
              <select
                ...
                className="bg-[#111118] border border-white/10 rounded-lg px-2 py-1 text-xs text-slate-300 focus:outline-none focus:border-cyan-500/50 cursor-pointer font-[family-name:var(--font-jetbrains)]"
              >
                {accounts.map((acc) => (
          ...
          <button
            onClick={onConnect}
            disabled={connecting}
            className="px-3 py-1.5 rounded-lg text-xs font-semibold bg-gradient-to-r from-purple-500 to-purple-400 text-white hover:from-purple-400 hover:to-purple-300 disabled:opacity-55 disabled:cursor-not-allowed transition-all duration-200 cursor-pointer"
            id="connect-wallet"
          >
